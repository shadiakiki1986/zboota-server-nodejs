  ...
  du.connect(function() {
    ...
    du.incrementPassFail(function() {
      ...
      MailManager.mailSend(
        ...
        { fail: function(err) { context.fail(angular.toJson({error: "Failed to send email to "+event.email})); },
...
module.exports.update = function (event,context) {
  ...
  lpns2=JSON.parse(event.lpns);
  for(k in lpns2) {
        v=lpns2[k];
  	if(!v.hasOwnProperty("a")||!v.hasOwnProperty("n")||v.a==""||v.n=="") {
          context.fail("Invalid area/number pair");
          return;
        }
        if(v.l=="") delete v.l; // drop empty labels because AWS putItem would fail
  }
