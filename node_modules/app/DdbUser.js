...
//          region: "us-west-2",
  ...
  this.checkEmailRegistered=function(exceptionIfNot) {
    ...
    if(exceptionIfNot) {
      if( !self.entry) { self.context.fail("Email address not registered."); return; }
    } else {
      if(!!self.entry) { self.context.fail("Email address already registered."); return; }
    }
  };
  ...
  this.checkPassFail=function() {
    ...
    if(pf>=MAX_PASS_FAIL) { self.context.fail("Account locked."); return; }
  };
  ...
  this.generatePassword=function() {
    ...
    self.event.pass=substr(uniqid(),-5,5);
  };
  ...
  this.newUser=function() {
    ...
    if(!mailValidate(self.event.email)) {
            self.context.fail("Invalid email "+self.event.email);
            return;
    }

    self.connect(function() {
      self.checkEmailRegistered(false); // throws an exception if the email exists
      self.generatePassword();
  
      // send email
      if(!mailSend(self.event.email,
              "Zboota registration",
              "Welcome to Zboota. Your password is "+self.event.pass
      )) {
              self.context.fail("Failed to send email to "+self.event.email);
              return;
      }
  ...
  this.loginCore=function(event,context,silent) {
    ...
    self.connect(function() {
      self.checkEmailRegistered(true);
      self.checkPassFail();
