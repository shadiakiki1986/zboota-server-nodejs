...
DdbGet.prototype.got = function(x, wa) {
  ...
  this.dynamo.getItem(
    ...
    function(err,data) {
      self.got2(err,data,x, wa);
    }
  );
}; // end got function

DdbGet.prototype.getFromWeb = function(data,x,existed,wa) {
  ...
  if(!getFromWeb) {
    ...
    ['dm','pml','isf'].forEach(function(k2) {
      if(data['Item'].hasOwnProperty(k2)) {
        if(k2=="dm") {
          getFromWeb=getFromWeb||(wa['dawlati'] && data['Item'][k2]=="Not available" && hasMech );
        } else {
          getFromWeb=getFromWeb||(wa[k2] && data['Item'][k2]=="Not available" );
        }
      }
    });
  ...
  //if(!this.silent) console.log("getFromWeb(",data.Item,x,existed,") ={",getFromWeb,",",hasMech,"}");
  return {g:getFromWeb,m:hasMech};
}; // end getFromWeb

DdbGet.prototype.got2 = function(err,data,x,wa) {
  var existed = data!=null && Object.keys(data).length!=0 || data.hasOwnProperty("Item");
  var temp = this.getFromWeb(data,x,existed,wa);
  var getFromWeb=temp.g;
  var hasMech=temp.m;
  var self=this;

  // if no need to get from web
  //if(!this.silent) console.log("got2 getFromWeb=",getFromWeb," isSync=",this.isSync," id=",data.Item.id);
  ...
  if(!this.silent) console.log("Refresh from web for: "+x.a+"/"+x.n);
...
DdbGet.prototype.get = function() {
  ...
  var du = new WebAvailability({
    ...
    succeed: function(wa) {
      if(self.isSync) {
        if(self.event.length==1) {
            self.got(self.event[0], wa);
        } else {
          // serial loop via callback so as not to overload the servers
          self.getCounter=0;
          self.concludeCb=function() {
            self.getCounter++;
            self.got(self.event[self.getCounter], wa);
          };
          self.got(self.event[self.getCounter], wa);
        }
      } else {
        ...
        self.event.forEach(function(x) {
          //if(!this.silent) console.log("get? "+x.a+", "+x.n);
          self.dynamo.getItem(
            { "TableName":"zboota-cars",
              "Key":{"id":x.a+"/"+x.n}
            },
