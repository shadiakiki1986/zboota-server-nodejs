...
var DdbGet = function(event,context,silent,isSync) {
...
DdbGet.prototype.got = function(err,data,x) {
  ...
  if(getFromWeb) {
    ...
    // \Todo update last get timestamp https://github.com/shadiakiki1986/zboota-server-nodejs/issues/3
    data=data.Item;
...
DdbGet.prototype.get = function() {
  var self=this;
  if(this.isSync) {
    // serial for loop so as not to overload the servers
    this.event=this.event.slice(0,2);// for testing: .. remove afterwards
    // this for loop should run self.got serially and wait for the web calls synchronously .. \TODO
    for(var i=0;i<this.event.length;i++) { self.got(null,null,this.event[i]); }
  } else {
    this.event.forEach(function(x) {
      self.dynamo.getItem(
        ...
        function(err,data) { self.got(err,data,x); }
      );
    });
  }
};
...
module.exports = function (event,context,silent,isSync) {
  return new DdbGet(event,context,silent,isSync);
};
