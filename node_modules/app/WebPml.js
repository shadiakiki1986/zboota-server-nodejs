...
exports.handler = function(data,callbackFn) {
  ...
  var du = new WebAvailability({
    ...
    succeed: function(res) {
      if(!res.pml) {
        callbackFn("PML website currently out of service");
        return;
      }
      ...
      request.get(options, function(err,res,body) {
        ...
        if(!res) { callbackFn("Not available"); return; }
        ...
        request.post(
          ...
          function (error, response, body) {
            ...
            if (error || response.statusCode != 200) { callbackFn("Not available"); return; }
            ...
            options.form={
              ...
              'ctl00$ContentPlaceHolder1$cmbCarType':Validator.mapPml()[data.a],
              'ctl00$ContentPlaceHolder1$btnSearch':'SUBMIT'
            };
    
            fBkp=options.form;
            options.form = querystring.stringify(options.form);
            // can skip? // options.form=options.form.replace(/B%2F%EF%BA%8F/,"B%2F%D8%A8");
            ...
            rereq=request.post(
                ...
                function (error, response, body) {
                  ...
                  if(!response) { callbackFn("Not available"); return; }
                  if (error || response.statusCode != 200) { callbackFn("Not available"); return; } // : "+error+","+response.statusCode);
