...
exports.handler = function(data,callbackFn) {
  ...
  var du = new WebAvailability({
    ...
    succeed: function(res) {
      if(!res.dawlati) {
        callbackFn("Mechanique website currently out of service");
        return;
      }
      ...
      request.get(options, function(err,res,body) {
        ...
        vdm = xpath.select('//td[@id="zVehiculeType"]//input[@name="_dawlatimecanique_WAR_dawlatimecaniqueportlet_vehicleType"]',doc)
        vdm = Validator.validDawlatiMechanique(vdm,data.t);
        if(!vdm.isvalid) {
          callbackFn("Invalid vehicle type: "+data.t+". Please use one of: "+JSON.stringify(vdm.list));
          return;
        }
      
        // Can no longer have
        //     vdm = xpath.select('//select[@id="_dawlatimecanique_WAR_dawlatimecaniqueportlet_modelYear"]//option/@value',doc)
        // because of https://github.com/goto100/xpath/issues/37
        vdm = xpath.select('//select[@id="_dawlatimecanique_WAR_dawlatimecaniqueportlet_modelYear"]//option',doc)
        vdm = Validator.validDawlatiMechanique(vdm,data.y); // @value doesn't work for some reason
        if(!vdm.isvalid) {
          callbackFn("Invalid model year "+data.y+". Please use one of: "+JSON.stringify(vdm.list));
          return;
        }
      
        // cannot select on '@value' ... check comment above
        vdm=xpath.select('//td[@id="zHorsePower"]//input[@name="_dawlatimecanique_WAR_dawlatimecaniqueportlet_horsePower"]',doc);
        vdm = Validator.validDawlatiMechanique(vdm,data.hp); // @value doesn't work for some reason
        if(!vdm.isvalid) {
          callbackFn("Invalid horse power: "+data.hp+". Please use one of: "+JSON.stringify(vdm.list));
          return;
        }
        ...
        request.post(
           ...
           function (error, response, body) {
            ...
            body = body.replace(/(\r\n|\n|\r)/gm,"");
            body=body.replace(/.*(<table id="zResult" >.*<\/table>).*/,"$1")
