    ...
    this.listCars({
      ...
      succeed: function(cars) {
        self.listUserCars({
          fail: context.fail,
          succeed: function(userCars) {
            var n=0; // number of dates at min/max
            var userCarIds=userCars.map(function(x) { return x.a+"/"+x.n; });
            cars=cars.filter(function(x) { return userCarIds.indexOf(x.a+"/"+x.n)!=-1; });
            ...
            var minDate = cars.map(function(x) {
              ...
              if(ismax) {
                if(a<b) { n=0; return b; } else if(a>b) { n=0; return a; } else { n++; return a; }
              } else {
                if(a<b) { n=0; return a; } else if(a>b) { n=0; return b; } else { n++; return a; }
              }
            },startDate);
            context.succeed({min: minDate, n: n});
          }
        });
      }
    });
