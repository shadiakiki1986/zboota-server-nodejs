      ...
      succeed: function(cars) {
        self.listUserCars({
          ...
          succeed: function(userCars) {
            ...
            cars=cars.filter(function(x) { return userCarIds.indexOf(x.a+"/"+x.n)!=-1; });
            if(cars.length==0) {
              context.fail("No user cars");
              return;
            }
            var minDate = cars.map(function(x) {
              return x.dataTs.substring(0,10);
            }).sort(); //increasing
            if(!ismax) minDate=minDate[0]; else minDate=minDate[minDate.length-1];
            var n = cars.filter(function(x) { return x.dataTs.substring(0,10)==minDate; }).length;
            context.succeed({min: minDate, n: n, t: cars.length});
          }
        });
      }
    });
